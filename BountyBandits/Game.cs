...
using BountyBandits.Graphics;
using BountyBandits.Graphics.Animation;
...
namespace BountyBandits
{
    public class Game : Microsoft.Xna.Framework.Game
    {
        ...
        #region Fields
        public const float FORCE_AMOUNT = 10, DROP_ITEM_MAX_DISTANCE = 4000f;
        ...
        public BountyBandits.Graphics.Animation.AnimationManager animationManager;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, 
                SamplerState.AnisotropicWrap, DepthStencilState.None, 
                    ...
                    else
                    {
                        ...
                        for (int currentDepth = 0; currentDepth < 4; currentDepth++)
                        {
                            foreach (GameItem gameItem in activeItems)
                                if (currentDepth >= gameItem.startdepth && currentDepth < gameItem.width)
                                    GraphicsHelper.drawGameItem(ref spriteBatch, ref texMan, this.GraphicsDevice, ref res, ref aveX, gameItem, currentDepth);
                            foreach (Being enemy in spawnManager.enemies)
                                if (currentDepth == enemy.getDepth())
                                    enemy.draw(ref spriteBatch, ref res, currentDepth);
                            foreach (Being player in players)
                                if (currentDepth == player.getDepth())
                                {
                                    player.draw(ref spriteBatch, ref res, currentDepth);
        ...
        public void drawItemDescription(DropItem item)
        {
            ...
            spriteBatch.Draw(texMan.getTex("portraitBackground"), new Vector2(item.body.Position.X - 46f - aveX, res.ScreenHeight - (item.body.Position.Y + 60f + (GraphicsHelper.DEPTH_MULTIPLE * (3 - item.startdepth)))), new Rectangle(0, 0, maxWidth, backgroundDrawHeight), new Color(255, 255, 255, 192));
            GraphicsHelper.drawItem(ref spriteBatch, ref res, texMan.getTexColored(item.getItem().getTextureName(), item.getItem().getPrimaryColor(), item.getItem().getSecondaryColor(), this.graphics.GraphicsDevice), new Vector2(item.body.Position.X - 25f - aveX, item.body.Position.Y + 15f), 0f, (int)item.startdepth, Vector2.One, SpriteEffects.None, new Vector2(texMan.getTex(item.getItem().getTextureName()).Width / 2, texMan.getTex(item.getItem().getTextureName()).Height / 2));
            spriteBatch.DrawString(vademecumFont12, name, new Vector2(item.body.Position.X - 10f - aveX, res.ScreenHeight - (item.body.Position.Y + 60f + (GraphicsHelper.DEPTH_MULTIPLE * (3 - item.startdepth)))), nameColor);
            spriteBatch.DrawString(vademecumFont12, stats, new Vector2(item.body.Position.X - 10f - aveX, res.ScreenHeight - (item.body.Position.Y + 45f - (20f * (name.Length / 20)) + (GraphicsHelper.DEPTH_MULTIPLE * (3 - item.startdepth)))), Color.White);
            #endregion
        }
