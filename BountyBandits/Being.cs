...
namespace BountyBandits
{
    public class Being
    {
        ...
        public Geom geom;
        public Controller controller; int currFrame;
        ...
        #endregion
        public Being(string name, int maxhealth, Game gameref, Controller controller)
        {
        ...
        public void draw(ref SpriteBatch spriteBatch, ref Resolution res, int currentDepth)
        {
            ...
            if (Environment.TickCount - timeOfLastDepthChange < timeToChangeDepths)
            {
                ...
                float slidex = GraphicsHelper.DEPTH_X_OFFSET / timeToChangeDepths * difference;   // 0 < x < DEPTH_X_OFFSET
                float slidey = GraphicsHelper.DEPTH_MULTIPLE / timeToChangeDepths * difference;   // 0 < y < DEPTH_MULTIPLE
                slidex *= directionMoving; slidey *= directionMoving;
                if (directionMoving == -1)
                {
                    slidex += GraphicsHelper.DEPTH_X_OFFSET;
                    slidey -= 2 * GraphicsHelper.DEPTH_MULTIPLE;
                }
                else
                {
                    slidex -= GraphicsHelper.DEPTH_X_OFFSET;
                    slidey -= 4 * GraphicsHelper.DEPTH_MULTIPLE;
                }
            ...
            GraphicsHelper.drawItem(ref spriteBatch, ref res, controller.frames[currFrame], new Vector2(drawPoint.X - gameref.getAveX() + gameref.res.ScreenWidth / 2, drawPoint.Y), 0, currentDepth, Vector2.One, effects, Vector2.Zero);
        }
        public int getDepth()
        {
